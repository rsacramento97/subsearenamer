...
pub enum SafeCopyError {
    ...
    #[error("insufficient disk space: required {required} bytes, available {available}")]
...
fn ensure_destination_is_outside_source(source: &Path, destination: &Path) -> Result<(), SafeCopyError> {
    let source_root = fs::canonicalize(source.parent().ok_or(SafeCopyError::InvalidSource)?)?;
    let destination_parent = canonical_parent(destination)?;
    if destination_parent != source_root && destination_parent.starts_with(&source_root) { return Err(SafeCopyError::DestinationInsideSource); }
    Ok(())
}
...
pub fn copy_verify_rename(source: &Path, destination: &Path, verify_hash: bool) -> Result<CopyReport, SafeCopyError> {
    ...
    if available < required { return Err(SafeCopyError::InsufficientSpace { required, available }); }
    ...
    let copy_result = (|| -> Result<u64, SafeCopyError> {
        let mut buffer = [0u8; 1024 * 1024]; let mut copied = 0u64;
        loop { let read = input.read(&mut buffer)?; if read == 0 { break; } output.write_all(&buffer[..read])?; copied += read as u64; }
        output.sync_all()?; Ok(copied)
    })();
    let copied = match copy_result { Ok(bytes) => bytes, Err(error) => { drop(output); let _ = fs::remove_file(&temp); return Err(error); } };
    drop(output);
    ...
    let source_hash = if verify_hash {
        let after = sha256_file(source)?; let before = source_hash_before.as_ref().expect("hash enabled"); let actual = sha256_file(&temp)?;
        if &after != before || &actual != before { let _ = fs::remove_file(&temp); return Err(SafeCopyError::IntegrityMismatch); }
        Some(after)
    } else { None };
...
mod tests {
    use super::*;
    #[test] fn bridge_contract_serializes() { let request=crate::bridge_contract::RenameJobRequest{source_dir:"C:/src".into(),destination_dir:"D:/dst".into(),timezone:"UTC".into(),manual_offset_minutes:Some(-180),verify_hash:true}; let value=serde_json::to_value(request).unwrap(); assert_eq!(value["sourceDir"],"C:/src"); assert_eq!(value["manualOffsetMinutes"],-180); }
    #[test] fn preview_status_serializes_stably() { assert_eq!(serde_json::to_string(&crate::bridge_contract::PreviewStatus::Ready).unwrap(),"\"ready\""); assert_eq!(serde_json::to_string(&crate::bridge_contract::PreviewStatus::Conflict).unwrap(),"\"conflict\""); }
}
